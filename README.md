...
Скрипт исправляет плохие оценки на пятерки, удаляет все замечания преподавателей к ученику и создает запись похвалы по указанному уроку.

## Использование скрипта

- в терминале запустите интерактивный режим Django shell  `python3 manage.py shell`
- в терминале Django shell подключите скрипт  `from main import *`

### Заменяем плохие оценки
...
`create_commendation('Фамилия Имя ученика', 'Название урока')`

Урок и похвала выбираются случайно. Текст похвалы находится в списке файла `main.py`

### Исправления в комплексе

Функция запускает все предыдущие исправления 

`main('Фамилия Имя ученика', 'Название урока')`
